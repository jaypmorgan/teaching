...
\begin{frame}[label={sec:org9f849e8},fragile]{The basic structure of importing}
...
\begin{frame}[label={sec:orge70f091},fragile]{Importing specific items}
...
\begin{frame}[label={sec:orgbc5bda5},fragile]{Don't do this!}
...
\begin{frame}[label={sec:org789db96},fragile]{Don't do this!}
...
\begin{frame}[label={sec:org4b9ac02},fragile]{Alias}
...
\begin{frame}[label={sec:orgb006a0b},fragile]{Importing local libraries}
...
\begin{frame}[label={sec:org29af9d1},fragile]{Importing local libraries}
 \begin{minted}[frame=lines,linenos=true,firstnumber=last,fontsize=\footnotesize,bgcolor=LightGray,xleftmargin=5pt,tabsize=2,breaklines=true,numbersep=10pt]{bash}
main.py
src/
 |-- my_module.py
...
\begin{frame}[label={sec:org7c2c23a},fragile]{Quick exercise -- imports}
 \begin{itemize}
...
\begin{frame}[label={sec:org1729baf},fragile]{Shortcuts with \texttt{\_\_init\_\_.py}}
...
\begin{frame}[label={sec:orgf840336},fragile]{What is \texttt{\_\_main\_\_}?}
...
\begin{frame}[label={sec:orgad1b77c},fragile]{What is \texttt{\_\_main\_\_}?}
...
\begin{frame}[label={sec:orgb6e773c},fragile]{What is \texttt{\_\_main\_\_}?}
...
\label{sec:org975eb02}

\subsection{Paths}
\label{sec:org211467f}

\begin{frame}[label={sec:orgddf7ec8},fragile]{Current working directory}
...
\begin{frame}[label={sec:orgb9cf081},fragile]{Listing directories}
...
\begin{frame}[label={sec:org9a5c774},fragile]{Testing for files or directories}
...
\begin{frame}[label={sec:org95dff3d},fragile]{Using wildcards}
...
\begin{frame}[label={sec:orgce626b5},fragile]{Pathlib -- a newer way}
...
\begin{frame}[label={sec:org930ad5d},fragile]{Pathlib -- convenient functions}
...
\begin{frame}[label={sec:org65b73d7},fragile]{Converting Path into a string}
...
\begin{frame}[label={sec:org866effd},fragile]{Quick exercise -- locating files}
 \begin{itemize}
...
\subsection{Files}
\label{sec:orgc8a8f7c}

\begin{frame}[label={sec:org06ddb5e},fragile]{Reading files}
...
\begin{frame}[label={sec:orgd20168b},fragile]{Reading files -- lines or entire file?}
...
\begin{frame}[label={sec:org2e40472},fragile]{Reading files}
...
\begin{frame}[label={sec:org8277009},fragile]{Writing files}
...
\begin{frame}[label={sec:orgd720f22},fragile]{Appending to files}
...
\begin{frame}[label={sec:org89c4700}]{Quick exercise -- reading/writing files}
\begin{itemize}
...
\begin{frame}[label={sec:org5a7a0de},fragile]{Reading CSV files -- builtin}
...
\begin{frame}[label={sec:org47099d7},fragile]{Writing a CSV file -- builtin}
...
\begin{frame}[label={sec:orgb916dca},fragile]{Quick exercise -- reading/writing CSV files}
 \begin{itemize}
...
\begin{frame}[label={sec:orgc5b3b97},fragile]{Read JSON files -- builtin}
...
\begin{frame}[label={sec:org90193bd},fragile]{Write JSON files -- builtin}
...
\label{sec:orgfb16912}

\subsection{Package Management}
\label{sec:org648dcf8}

\begin{frame}[label={sec:orgf140d5e}]{Introduction}
...
\begin{frame}[label={sec:org34ef3c7}]{Virtual Environments}
...
\subsection{Anaconda}
\label{sec:orgc9fd179}

\begin{frame}[label={sec:orga3cb47f},fragile]{What is Anaconda?}
 \begin{center}
...
\begin{frame}[label={sec:orgd66b8d5},fragile]{Installing Anaconda}
...
\begin{frame}[label={sec:orga1bb30b},fragile]{Working with Anaconda -- creating an environment}
...
\begin{frame}[label={sec:orgd062af4},fragile]{Working with Anaconda -- activating an environment}
...
\begin{frame}[label={sec:org6485a5b},fragile]{Working with Anaconda -- de-activating an environment}
...
\begin{frame}[label={sec:orgf21d98a},fragile]{Working with Anaconda -- installing using conda}
...
\begin{frame}[label={sec:org3dc3899},fragile]{Working with Anaconda -- package versions}
 \begin{minted}[frame=lines,linenos=true,firstnumber=last,fontsize=\footnotesize,bgcolor=LightGray,xleftmargin=5pt,tabsize=2,breaklines=true,numbersep=10pt]{bash}
...
\begin{frame}[label={sec:orgd0626c4},fragile]{Installing a specific version of Python}
...
\begin{frame}[label={sec:org2a1fcdf},fragile]{Working with Anaconda -- conda-forge and other repositories}
...
\begin{frame}[label={sec:org6ce6ae7},fragile]{Working with Anaconda -- exporting an environment}
...
\begin{frame}[label={sec:org3982491},fragile]{Working with Anaconda -- creating environment from existing}
...
\subsection{Pip}
\label{sec:orga2710d2}

\begin{frame}[label={sec:org21ff29d}]{What is Pip?}
...
\begin{frame}[label={sec:orgde5f4cb},fragile]{Installing packages with pip}
...
\begin{frame}[label={sec:orgfb15a09},fragile]{Installing specific versions}
...
\begin{frame}[label={sec:org61a9e13},fragile]{Upgrade packages with pip}
...
\begin{frame}[label={sec:org5d7fa46},fragile]{Export requirements file}
...
\begin{frame}[label={sec:orgea2b938},fragile]{Installing multiple packages from a requirements file}
...
\begin{frame}[label={sec:org11a18c9},fragile]{Anaconda handles both conda and pip}
...
\label{sec:org1566725}

\subsection{PyCharm}
\label{sec:orgb6b9c7b}

\begin{frame}[label={sec:orga8be704}]{PyCharm}
...
\begin{frame}[label={sec:orgb60246b},fragile]{PyCharm -- installing}
 Using Ubuntu snaps:
...
\begin{frame}[label={sec:org7852e31}]{PyCharm -- using PyCharm}
...
\subsection{Jupyter}
\label{sec:org1179339}

\begin{frame}[label={sec:org1864df2}]{What is a Jupyter notebook?}
\begin{center}
...
\begin{frame}[label={sec:orgca06b8a},fragile]{Installing Jupyter}
...
\begin{frame}[label={sec:orgc0fb533},fragile]{Starting the server}
...
\begin{frame}[label={sec:org25dfaa8}]{Using the interface}
We shall take a look at the following:
...
\begin{frame}[label={sec:org361274e}]{Markdown 101}
...
\begin{frame}[label={sec:org5a8fbcc},fragile]{A slightly better environment -- jupyterlab}
...
\label{sec:org3b20442}

\subsection{Styles}
\label{sec:org78e01da}

\begin{frame}[label={sec:orge437c24}]{A sense of style}
...
\begin{frame}[label={sec:orga088c06},fragile]{Meaningful names}
...
\begin{frame}[label={sec:orgd11de9c},fragile]{Meaningful names}
 What about this one?
...
\begin{frame}[label={sec:orgce8cbda},fragile]{Use builtins where possible}
...
\begin{frame}[label={sec:org31c67a1},fragile]{Use docstrings and comments}
 \begin{minted}[frame=lines,linenos=true,firstnumber=last,fontsize=\footnotesize,bgcolor=LightGray,xleftmargin=5pt,tabsize=2,breaklines=true,numbersep=10pt]{python}
...
\begin{frame}[label={sec:orge642c49},fragile]{Using agreed upon casing}
 \begin{itemize}
...
\begin{frame}[label={sec:org6139010},fragile]{Use type-annotations if possible}
...
\begin{frame}[label={sec:org6656959},fragile]{Organise your imports}
...
\begin{frame}[label={sec:org4336014}]{Functions should do one thing only}
...
\begin{frame}[label={sec:orgb4111dd}]{Functions as re-usability}
...
\begin{frame}[label={sec:orgd44a909}]{Be wary of God classes}
...
\begin{frame}[label={sec:org17ef946},fragile]{Documentation}
 \begin{quote}
...
\begin{frame}[label={sec:orge361458},fragile]{Perform testing!}
