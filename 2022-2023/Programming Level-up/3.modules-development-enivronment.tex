...
\begin{frame}[label={sec:org80f734a},fragile]{The basic structure of importing}
...
\begin{frame}[label={sec:orgebb91e5},fragile]{Importing specific items}
...
\begin{frame}[label={sec:orgf0a470d},fragile]{Don't do this!}
...
\begin{frame}[label={sec:orgee1aae4},fragile]{Don't do this!}
...
\begin{frame}[label={sec:org665ff3a},fragile]{Alias}
...
\begin{frame}[label={sec:org1e140c3},fragile]{Importing local libraries}
...
\begin{frame}[label={sec:orgc05ef82},fragile]{Importing local libraries}
 \begin{minted}[frame=lines,linenos=true,firstnumber=last,fontsize=\footnotesize,bgcolor=LightGray,xleftmargin=5pt,tabsize=2,breaklines=true,numbersep=10pt]{bash}
main.py
src/
 |-- my_module.py
...
\begin{frame}[label={sec:org53ab93e},fragile]{Quick exercise -- imports}
 \begin{itemize}
...
\begin{frame}[label={sec:org4c47994},fragile]{Shortcuts with \texttt{\_\_init\_\_.py}}
...
\begin{frame}[label={sec:org709258c},fragile]{What is \texttt{\_\_main\_\_}?}
...
\begin{frame}[label={sec:org1af9e45},fragile]{What is \texttt{\_\_main\_\_}?}
...
\begin{frame}[label={sec:org278a414},fragile]{What is \texttt{\_\_main\_\_}?}
...
\label{sec:org2a19e46}

\subsection{Paths}
\label{sec:org320c887}

\begin{frame}[label={sec:org632c5c5},fragile]{Current working directory}
...
\begin{frame}[label={sec:org2a267b0},fragile]{Listing directories}
...
\begin{frame}[label={sec:orge4860ef},fragile]{Testing for files or directories}
...
\begin{frame}[label={sec:org6490edf},fragile]{Using wildcards}
...
\begin{frame}[label={sec:orgae5fce2},fragile]{Pathlib -- a newer way}
...
\begin{frame}[label={sec:orgb47784a},fragile]{Pathlib -- convenient functions}
...
\begin{frame}[label={sec:orgabe3ee8},fragile]{Converting Path into a string}
...
\begin{frame}[label={sec:org05c9c71},fragile]{Quick exercise -- locating files}
 \begin{itemize}
...
\subsection{Files}
\label{sec:org2de94b6}

\begin{frame}[label={sec:orga398793},fragile]{Reading files}
...
\begin{frame}[label={sec:org3ae0b58},fragile]{Reading files -- lines or entire file?}
...
\begin{frame}[label={sec:org107af42},fragile]{Reading files}
...
\begin{frame}[label={sec:org1ec8e0a},fragile]{Writing files}
...
\begin{frame}[label={sec:org3501ff0},fragile]{Appending to files}
...
\begin{frame}[label={sec:orgd8bce8a}]{Quick exercise -- reading/writing files}
\begin{itemize}
...
\begin{frame}[label={sec:org8d5a9e5},fragile]{Reading CSV files -- builtin}
...
\begin{frame}[label={sec:org2d22efa},fragile]{Writing a CSV file -- builtin}
...
\begin{frame}[label={sec:orgea290b5},fragile]{Quick exercise -- reading/writing CSV files}
 \begin{itemize}
...
\begin{frame}[label={sec:org1571010},fragile]{Read JSON files -- builtin}
...
\begin{frame}[label={sec:orgf2eac2a},fragile]{Write JSON files -- builtin}
...
\label{sec:org421bd5c}

\subsection{Package Management}
\label{sec:org46e42f9}

\begin{frame}[label={sec:orgb85a298}]{Introduction}
...
\begin{frame}[label={sec:org7672a9e}]{Virtual Environments}
...
\subsection{Anaconda}
\label{sec:org9c85511}

\begin{frame}[label={sec:org07aeae0},fragile]{What is Anaconda?}
 \begin{center}
...
\begin{frame}[label={sec:org13184f7},fragile]{Installing Anaconda}
...
\begin{frame}[label={sec:orgac1acf8},fragile]{Working with Anaconda -- creating an environment}
...
\begin{frame}[label={sec:org9ba9e08},fragile]{Working with Anaconda -- activating an environment}
...
\begin{frame}[label={sec:orgebab571},fragile]{Working with Anaconda -- de-activating an environment}
...
\begin{frame}[label={sec:orgdc57b4b},fragile]{Working with Anaconda -- installing using conda}
...
\begin{frame}[label={sec:org55fdaac},fragile]{Working with Anaconda -- package versions}
 \begin{minted}[frame=lines,linenos=true,firstnumber=last,fontsize=\footnotesize,bgcolor=LightGray,xleftmargin=5pt,tabsize=2,breaklines=true,numbersep=10pt]{bash}
...
\begin{frame}[label={sec:org0eab57c},fragile]{Installing a specific version of Python}
...
\begin{frame}[label={sec:org4cde86c},fragile]{Working with Anaconda -- conda-forge and other repositories}
...
\begin{frame}[label={sec:orgc5c3111},fragile]{Working with Anaconda -- exporting an environment}
...
\begin{frame}[label={sec:org86fdb39},fragile]{Working with Anaconda -- creating environment from existing}
...
\subsection{Pip}
\label{sec:orgcccb7d5}

\begin{frame}[label={sec:org58ce6d1}]{What is Pip?}
...
\begin{frame}[label={sec:org6612be0},fragile]{Installing packages with pip}
...
\begin{frame}[label={sec:org37afad6},fragile]{Installing specific versions}
...
\begin{frame}[label={sec:org8a393f1},fragile]{Upgrade packages with pip}
...
\begin{frame}[label={sec:orga654f08},fragile]{Export requirements file}
...
\begin{frame}[label={sec:orgf8efa39},fragile]{Installing multiple packages from a requirements file}
...
\begin{frame}[label={sec:orgdfd9ff5},fragile]{Anaconda handles both conda and pip}
...
\label{sec:org4c8721e}

\subsection{PyCharm}
\label{sec:orge1745bb}

\begin{frame}[label={sec:org758aade}]{PyCharm}
...
\begin{frame}[label={sec:org08ab9f1},fragile]{PyCharm -- installing}
 Using Ubuntu snaps:
...
\begin{frame}[label={sec:orgedcbf74}]{PyCharm -- using PyCharm}
...
\subsection{Jupyter}
\label{sec:org12c5faa}

\begin{frame}[label={sec:orgc9af4f1}]{What is a Jupyter notebook?}
\begin{center}
...
\begin{frame}[label={sec:org72b1a5e},fragile]{Installing Jupyter}
...
\begin{frame}[label={sec:orgbf74562},fragile]{Starting the server}
...
\begin{frame}[label={sec:orgcca5349}]{Using the interface}
We shall take a look at the following:
...
\begin{frame}[label={sec:org1556510}]{Markdown 101}
...
\begin{frame}[label={sec:orgba1a63b},fragile]{A slightly better environment -- jupyterlab}
...
\label{sec:org47f4aa8}

\subsection{Styles}
\label{sec:org1830754}

\begin{frame}[label={sec:org5c1920d}]{A sense of style}
...
\begin{frame}[label={sec:org840980b},fragile]{Meaningful names}
...
\begin{frame}[label={sec:org12ddfa7},fragile]{Meaningful names}
 What about this one?
...
\begin{frame}[label={sec:org3c848b8},fragile]{Use builtins where possible}
...
\begin{frame}[label={sec:org18b32cd},fragile]{Use docstrings and comments}
 \begin{minted}[frame=lines,linenos=true,firstnumber=last,fontsize=\footnotesize,bgcolor=LightGray,xleftmargin=5pt,tabsize=2,breaklines=true,numbersep=10pt]{python}
...
\begin{frame}[label={sec:org73aea7d},fragile]{Using agreed upon casing}
 \begin{itemize}
...
\begin{frame}[label={sec:org57f2e3c},fragile]{Use type-annotations if possible}
...
\begin{frame}[label={sec:org1d6663a},fragile]{Organise your imports}
...
\begin{frame}[label={sec:org03f45bd}]{Functions should do one thing only}
...
\begin{frame}[label={sec:org0069158}]{Functions as re-usability}
...
\begin{frame}[label={sec:orgff5918d}]{Be wary of God classes}
...
\begin{frame}[label={sec:org1f8c818},fragile]{Documentation}
 \begin{quote}
...
\begin{frame}[label={sec:org1cf0ded},fragile]{Perform testing!}
